...
class FileManager implements FileManagerInterface
{
  ...
  public function createImages($source, $filename, $cat=2, $processing=True): bool
  {
    ...
    foreach($this->imagetypes as $key => $imagetype)
    {
      ...
      if(!$error)
      {
        ...
        // $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_SUCCESS_CREATE_IMAGETYPE', $filename, $imagetype->typename), 'info', 'jerror');
      }
    }
    ...
    if($error)
    {
      ...
      $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_SOME_ERRORS_IMAGEFILE'), 'error', 'jerror');
  ...
  public function deleteImages($img): bool
  {
    ...
    foreach($this->imagetypes as $key => $imagetype)
    {
      ...
      // $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_SUCCESS_DELETE_IMAGETYPE', \basename($file), $imagetype->typename), 'info', 'jerror');
    }
  ...
  public function moveImages($img, $dest, $filename=false, $copy=false): bool
  {
    ...
    foreach($this->imagetypes as $key => $imagetype)
    {
      ...
      catch(\FileNotFoundException $e)
      {
        ...
        $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_ERROR_FILE_NOT_EXISTING'.', '.\basename($img_src).' ('.$imagetype->typename.')'), 'error', 'jerror');
        $error = true;
  ...
  public function renameImages($img, $filename): bool
  {
    ...
    foreach($this->imagetypes as $key => $imagetype)
    {
      ...
      catch(\FileNotFoundException $e)
      {
        ...
        $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_ERROR_FILE_NOT_EXISTING').', '.\basename($file_orig).' ('.$imagetype->typename.')', 'error', 'jerror');
        $error = true;
    ...
    if($error)
    {
      ...
      $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_ERROR_RENAME_IMAGE', \basename($file_orig)), 'error', 'jerror');
    ...
    $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_SUCCESS_RENAME_IMAGE', \basename($file_orig)), 'error', 'jerror');
  ...
  public function deleteCategory($cat, $del_images=false): bool
  {
    ...
    foreach($this->imagetypes as $key => $imagetype)
    {
      ...
      if(\count($files) > 0)
      {
        if(!$del_images)
        {
          ...
          $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_ERROR_DELETE_CATEGORY_NOTEMPTY', \basename($path)), 'error', 'jerror');
    ...
    $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_SUCCESS_DELETE_CATEGORY', \basename($path)), 'error', 'jerror');
  ...
  public function moveCategory($cat, $dest, $foldername=false, $copy=false): bool
  {
    ...
    $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_SUCCESS_').$method.'_CATEGORY', \basename($src_path), 'error', 'jerror');
  ...
  public function renameCategory($cat, $foldername): bool
  {
    ...
    $this->component->addLog(Text::sprintf('COM_JOOMGALLERY_SERVICE_SUCCESS_RENAME_CATEGORY'), \basename($folder_orig), 'error', 'jerror');
    ...
    if($catid === false || $filename === false)
    {
      ...
      elseif((\is_numeric($img) && $img > 0) || (\is_string($img) && !$this->is_path($img)))
      {
        ...
        if($img === false || \is_null($img->id))
        {
          ...
          $this->component->addLog(Text::_('COM_JOOMGALLERY_SERVICE_ERROR_GETIMGPATH', $img), 'error', 'jerror');
      ...
      else
      {
        ...
        $this->component->addLog(Text::_('COM_JOOMGALLERY_SERVICE_ERROR_GETPATH_NOQUERY', 'Image'), 'error', 'jerror');
  ...
  public function getCatPath($cat, $type=false, $parent=false, $alias=false, $root=false, $compatibility=true)
  {
    ...
    elseif((\is_numeric($cat) && $cat > 0) || (\is_string($cat) && \intval($cat) > 0))
    {
      ...
      if($cat === false)
      {
        ...
        $this->component->addLog(Text::_('COM_JOOMGALLERY_SERVICE_ERROR_GETCATPATH', $cat), 'error', 'jerror');
    ...
    elseif($parent && $alias)
    {
      ...
      elseif(\is_numeric($parent) || \is_string($parent))
      {
        ...
        if($parent === false)
        {
          ...
          $this->component->addLog(Text::_('COM_JOOMGALLERY_SERVICE_ERROR_GETCATPATH', $parent), 'error', 'jerror');
    ...
    else
    {
      ...
      $this->component->addLog(Text::_('COM_JOOMGALLERY_SERVICE_ERROR_GETPATH_NOQUERY', 'Category'), 'error', 'jerror');
